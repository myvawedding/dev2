 ...
 * @return {[type]} [description]
 */
GMW_Map.prototype.getIwAjaxContent = function( locationId, marker ) {

	var self = this;

	var location_data = self.locations[ locationId ];
	...
	jQuery.ajax( {
		...
		success	: function( content ) {
			...
			jQuery( content ).appendTo( '.gmw-info-window.map-' + self.id );
	...
	GMW_Map_Providers.google_maps.markerGroupingTypes.markers_spiderfier = {
		...
		'init' : function( mapObject ) {
		    ...
		    if ( typeof OverlappingMarkerSpiderfier === 'function' ) {
				...
				options = GMW.apply_filters( 'gmw_map_spiderfire_options', options, self );
		...
		'clear' : function( mapObject ) {
			// nothing todo. Spiderfires cleared when marker removed from the map.
		},
	...
	GMW_Map_Providers.google_maps.infoWindowTypes = {
		...
		standard : {
			...
			open : function( marker, mapObject ) {
			    ...
			    if ( self.infoWindowAjax == 1 ) {
					self.getIwAjaxContent( marker.location_id, marker );
				}
			},
...
if ( typeof( GMW_Map_Providers.leaflet ) !== 'undefined' ) {
	...
	GMW_Map_Providers.leaflet.markerGroupingTypes.markers_spiderfier = {
		...
		// Nothing to clear. Spiderfires cleared when markers are removed from the map.
		'clear' : function( mapObject ) {},
		...
		standard : {
			...
			open : function( marker, mapObject ) {
				...
				if ( self.infoWindowAjax == 1 ) {
					...
					content = marker.options.iw_content;
				}
			    ...
			    // get content via ajax
			    if ( self.infoWindowAjax == 1 ) {
					self.getIwAjaxContent( marker.options.location_id, marker );
				}
			},
