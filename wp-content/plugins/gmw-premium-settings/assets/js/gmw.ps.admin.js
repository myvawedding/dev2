jQuery( document ).ready( function($) {
	...
	// taxonomies show/hide
	// on/off click to make sure it doesn't clash with other addons with same feature
	$( '#search_form-taxonomies-tr .taxonomy-wrapper .taxonomy-header' ).off( 'click' ).on( 'click', function() {

		var taxonomy = $( this ).closest( '.taxonomy-wrapper' );
		var taxId    = taxonomy.closest( 'div' ).attr( 'id' );

		// hide all taxonomies
		$( '#search_form-taxonomies-tr .taxonomy-wrapper .taxonomy-settings:not( #' + taxId + ')' ).slideUp( 'fast' );
		// show selected taxonomies group
		taxonomy.find( '.taxonomy-settings:hidden' ).slideDown();
	});

	$( '#search_form-taxonomies-tr' ).find( 'select.taxonomy-usage' ).change( function() {
		$( this ).closest( '.taxonomy-settings-table-wrapper' ).attr( 'data-type', $( this ).val() );
	});
