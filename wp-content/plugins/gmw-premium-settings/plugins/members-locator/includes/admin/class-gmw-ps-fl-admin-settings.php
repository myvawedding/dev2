...
class GMW_PS_FL_Admin_Settings {
	...
	public function displayed_member_location_tab( $value, $name_attr ) {
		...
		<label>
			...
			>
			<span>
				<?php _e( 'Enable Location tab', 'gmw-premium-settings' ); ?>
			</span>
		</label>
		</p>

		<div <?php echo $display; ?> id="displayed-user-tab-content">

			<div style="float: left;">

				<label><b><?php _e( 'Elements', 'gmw-premium-settings' ); ?></b></label>
				...
				<ul>
					...
					foreach ( $items as $name => $label ) { 
						...
						$checked = ( ! empty( $value['elements'] ) && in_array( $name, $value['elements'] ) ) ? 'checked="checked"' : ''; 
						$onclick = $name == 'address' ? 'onchange="jQuery(\'#displayed-user-tab-address-fields\' ).slideToggle();"' : '';
						?>	
						<li>
							<label>
								<input 
			...
			<?php 
				if ( is_array( $value['elements'] ) && in_array( 'address', $value['elements'] ) ) {
					$display = 'style="float: left; margin-left: 50px"';
				} else {
					$display = 'style="display:none;float: left; margin-left: 50px"';
				}
			?>
			<div <?php echo $display; ?> id="displayed-user-tab-address-fields">

				<label><b><?php _e( 'Address fields', 'gmw-premium-settings' ); ?></b></label>
				...
				<ul>
					...
					foreach ( $items as $name => $label ) { 
						...
						$checked = ( ! empty( $value['address_fields'] ) && in_array( $name, $value['address_fields'] ) ) ? 'checked="checked"' : ''; 
						?>	
						<li>
							<label>
								<input 
