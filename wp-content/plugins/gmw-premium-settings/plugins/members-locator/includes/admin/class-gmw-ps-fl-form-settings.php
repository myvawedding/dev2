...
class GMW_PS_FL_Form_Settings {
    ...
    public function default_settings( $settings, $args ) {
        ...
        if ( $form['component'] == 'members_locator' ) {
	 ...
	 * @return [type]           [description]
	 */
	function form_settings( $form_fields, $form ) {
    ...
    public function include_exclude_bp_member_types( $value, $name_attr ) {
        ...
        if ( empty( $member_types ) ) {
            ?>
            <p><?php _e( 'No member types were found.', 'gmw-premium-settings' ); ?></p>
        ...
        <div class="gmw-options-box bp-include-exclude-member-types-settings-wrapper">  
            ...
            <div class="single-option usage">   
                ...
                <div class="option-content">
                    ...
                    <select name="<?php echo esc_attr( $name_attr ); ?>[usage]">
                        ...
                        <option value="disabled" selected="selected"><?php _e( 'Disabled', 'gmw-premium-settings' ); ?></option>

                        <option value="include" <?php if ( $value['usage'] == 'include' ) echo 'selected="selected"'; ?>><?php _e( 'Include', 'gmw-premium-settings' ); ?></option>
                        
                        <option value="exclude" <?php if ( $value['usage'] == 'exclude' ) echo 'selected="selected"'; ?>><?php _e( 'Exclude', 'gmw-premium-settings' ); ?></option>
                    </select>
                    
                    <p class="description">
                        <?php _e( 'Select to disable, include or exclude member type.', 'gmw-premium-settings' ); ?>
                    </p>
                </div>
            </div>
            ...
            <div class="single-option terms">   
                ...
                <label><?php _e( 'Member types to include or exclude', 'gmw-premium-settings' ); ?></label>  
                
                <div class="option-content">

                    <?php $set_taxes = ! empty( $value['member_types'] ) ? $value['member_types'] : array(); ?>

                    <select multiple name="<?php echo $name_attr; ?>[member_types][]">

                        <?php foreach ( $member_types as $member_type ) { ?>
                                                        
                            <?php $selected = ( isset( $value['member_types'] ) && in_array( $member_type->name, $value['member_types'] ) ) ? 'selected="selected"' : ''; ?>
                            
                            <option <?php echo $selected; ?> value="<?php echo esc_attr( $member_type->name ); ?>"><?php echo esc_html( $member_type->labels['name'] ); ?></option>
                        <?php } ?>

                    </select>

                    <p class="description">
                        <?php _e( 'Select the member types to include or exclude.', 'gmw-premium-settings' ); ?>
                    </p>
                </div>
    ...
    public static function bp_member_types( $value, $name_attr, $form ) {
        ...
        <div class="bp-member-types-settings-wrapper gmw-options-box">
            ...
            <div class="bp-member-types-setting single-option usage">  
                ...
                <label>
                    <?php _e( 'Usage', 'gmw-premium-settings' ); ?>      
                </label>    
                ...
                <div class="option-content">
                    ...
                    <select name="<?php echo esc_attr( $name_attr ); ?>[usage]" id="bp-member-types-usage">
                        ...
                        <option value="pre_defined" <?php if ( $value['usage'] == 'pre_defined' ) echo 'selected="selected"'; ?>><?php _e( 'Pre-defined', 'gmw-premium-settings' ); ?></option>
                    ...
                    <p class="description">
                        <?php _e( 'Select "Pre defined" to omit the filter from the search form, but your can still filter member types based on the selected types below. Otherwise, select how to display the filter in the search from.', 'gmw-premium-settings' ); ?>
                    </p>
                </div>
            </div>
            ...
            <div class="bp-member-types-setting single-option members">
                <label>
                    <?php echo _e( 'Select Member Types', 'gmw-premium-settings' ); ?> 
                </label>
                <div class="option-content">
                    ...
                    <p class="description">
                        <?php _e( 'Select member types to use in the filter or leave blank to use all types.', 'gmw-premium-settings' ); ?>
                    </p>
                </div>
            </div>
            ...
            <div class="single-option label">
                ...
                <div class="option-content">
                    ...
                    />
                    <p class="description">
                        <?php _e( 'Enter field label or leave blank to omit.', 'gmw-premium-settings' ); ?>
                    </p>
                </div>
            </div>
    ...
    public static function bp_groups( $value, $name_attr, $form ) {
        ...
        if ( ! class_exists( 'Buddypress' ) || ! bp_is_active( 'groups' ) ) {
            return _e( 'Buddypress Groups component is required for this feature.', 'gmw-premium-settings');
        }
        ...
        <div class="bp-groups-settings-wrapper gmw-options-box">
        	...
        	<div class="bp-group-setting single-option usage">	
   				...
   				<div class="option-content">
					...
					<p class="description">
						<?php _e( 'Select "Pre defined" to omit the filter from the search form, but still filter members based on the selected groups below. Otherwise, select how to display the filter in the search from.', 'gmw-premium-settings' ); ?>
					</p>
				</div>
			</div>
   			...
   			<div class="bp-group-setting single-option groups">
   				<label>
   					<?php echo _e( 'Select Groups', 'gmw-premium-settings' ); ?>	
   				</label>
   				<div class="option-content">
			        <select multiple="multiple" data-placeholder="Select some groups..." name="<?php echo esc_attr( $name_attr ); ?>[groups][]" id="" class="bp-groups-selector">
			            <?php foreach ( $groups as $group ) { ?>
			            	...
			            	<?php $selected = ( isset( $value['groups'] ) && in_array( $group->id, $value['groups'] ) ) ? 'selected="selected"' : ''; ?>
			            	
			            	<option <?php echo $selected; ?> value="<?php echo absint( $group->id ); ?>"><?php echo esc_html( $group->name ); ?></option>
			        	<?php } ?>
			        </select>
			        <p class="description">
						<?php _e( 'Select groups to use in the filter or leave blank to use all groups.', 'gmw-premium-settings' ); ?>
					</p>
			    </div>
	    	</div>
