 ...
 */
function gmw_ps_fl_location_form_args( $args ) {
...
function gmw_fl_filter_bp_groups_member_query( $query_args, $form ) {
     ...
     */
    } else if ( isset( $form['search_form']['bp_groups']['groups'] ) && array_filter( $form['search_form']['bp_groups']['groups'] ) ) {
    ...
    } else if ( ! empty( $query_args['include'] ) ) {
...
function gmw_ps_disable_displayed_user_location_tab( $args ) {
    ...
    if ( gmw_get_option( 'members_locator', 'displayed_member_location_tab_elements', array() ) == false ) {
...
function gmw_ps_displayed_user_location_tab( $content ) {
    ...
    if ( ( $elements = gmw_get_option( 'members_locator', 'displayed_member_location_tab_elements', false ) ) === false ) {
        return;
    }
    
    $elements       = implode( ',', $elements );
    $address_fields = gmw_get_option( 'members_locator', 'displayed_member_location_tab_address_fields', array( 'address' ) ); 
    $address_fields = ( in_array( 'address', $address_fields ) || count( $address_fields ) == 6 ) ? 'formatted_address' : implode( ',', $address_fields );
 ...
 * 
 * @param  [type] $activity_address [description]
...
function gmw_ps_member_activity_address_fields( $activity_address, $user_location, $user_id ) {
    ...
    } else if ( in_array( 'address', $address_fields ) ) {
