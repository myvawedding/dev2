...
class GMW_PS_PT_Form_Settings {
	...
	public function __construct() {
		...
		add_filter( 'gmw_validate_form_settings_ps_pt_include_exclude_terms', array( $this, 'validate_include_exclude_terms' ), 15 );
	}
    ...
    public function default_settings( $settings, $form ) {
        ...
        if ( $form['component'] == 'posts_locator' ) {
	...
	public function form_settings( $form_fields, $form ) {
		...
		$form_fields['search_form']['custom_fields'] = array(
			...
			'desc'       	=> __( '<p>Using this feature you can add custom fields filters to the search form. At the moment, the field can only be used as textboxes.</p><p> - Click the cog icon to manage the custom fields.</p><p> - Select a field from the dropdown menu, then click "add field".</p><p> - Select the field type and the compration that you would like to use.</p><p> - Enter field lable or leave blank to ommit the label.</p><p> - Enter a placeholder or leave blank to ommit it.</p>', 'gmw-premium-settings' ),
			'attributes'    => array(),
    		'priority'      => 16  
		);
    ...
    public static function post_types_settings( $value, $name_attr, $form ) {
        ...
        <div class="posts-types-settings-wrapper gmw-options-box" style="display:none;margin-top: 20px;">	
   			<div class="post-type-setting single-option usage">	
   				<label><?php _e( 'Usage', 'gmw-premium-settings' ); ?></label>	
   				<?php 
   				...
   				<div class="option-content">
					<select name="<?php echo esc_attr( $name_attr ); ?>[usage]" style="width:100%">
						<option value="pre_defined" selected="selected"><?php _e( 'Pre-defined', 'gmw-premium-settings' ); ?></option>
						<option value="dropdown" <?php if ( $value['usage'] == 'dropdown' ) echo 'selected="selected"'; ?>><?php _e( 'Dropdown', 'gmw-premium-settings' ); ?></option>
						<option value="checkboxes" <?php if ( $value['usage'] == 'checkboxes' ) echo 'selected="selected"'; ?>><?php _e( 'Checkboxes', 'gmw-premium-settings' ); ?></option>
						<option value="smartbox" <?php if ( $value['usage'] == 'smartbox' ) echo 'selected="selected"'; ?>><?php _e( 'Smartbox', 'gmw-premium-settings' ); ?></option>
						<option value="smartbox_multiple" <?php if ( $value['usage'] == 'smartbox_multiple' ) echo 'selected="selected"'; ?>><?php _e( 'Smartbox ( multiple selections )', 'gmw-premium-settings' ); ?></option>
					</select>
				</div>
			</div>
            ...
            <div class="single-option label half">
				...
				<div class="option-content">
					<input 
						type="text" 
						placeholder="<?php _e( 'Field label', 'gmw-premium-settings' ); ?>" 
			...
			<div class="single-option options-all half">
				...
				<label><?php _e( 'Options all label', 'gmw-premium-settings'); ?></label>	

				<div class="option-content">
					<input 
						type="text" 
						placeholder="<?php _e( 'Options all label', 'gmw-premium-settings' ); ?>" 
	...
	public static function validate_post_types_settings( $output ) {
		...
		if ( ! isset( $output['usage'] ) || ! in_array( $output['usage'], $options ) ) {
	 ...
	 * 
	 * @param  [type] $gmw_forms [description]
		...
		<div id="taxonomies-wrapper">
			...
			<?php foreach ( $posts as $post ) : ?>
				...
				<?php if ( ! empty( $post_taxonomies ) ) { ?>
					...
					<?php $style = ( isset( $form['search_form']['post_types'] ) && ( count( $form['search_form']['post_types'] ) == 1 ) && ( in_array( $post, $form['search_form']['post_types'] ) ) ) ? '' : 'style="display:none"'; ?>
					...
					<div id="post-type-<?php echo esc_attr( $post ); ?>-taxonomies-wrapper" class="post-type-taxonomies-wrapper" <?php echo $style; ?>>	
						...
						?>
						<?php foreach ( $post_taxonomies as $key => $taxonomy_name ) :
								...
								<div class="gmw-options-box taxonomy-settings-table-wrapper taxonomy-settings" data-type="<?php echo esc_attr( $style ); ?>">
									...
									<div style="display: inline-block;">
										...
										<div class="single-option">
											...
											<label><?php echo _e( 'Usage', 'gmw-premium-settings' ); ?></label>

											<div class="taxonomy-usage taxonomy-tab-content option-content">					
												
												<select name="<?php echo $tax_name_attr; ?>[style]" class="taxonomy-usage">
													...
													<option 
														value="dropdown" 
														<?php if ( $tax_option['style'] == 'dropdown' ) echo  'selected="selected"'; ?>
													>
														<?php _e( 'Dropdown','gmw-premium-settings' ); ?>
													</option>
													<option 
														value="smartbox" 
														<?php if ( $tax_option['style'] == 'smartbox' ) echo  'selected="selected"'; ?>
													> 
														<?php _e( 'Smartbox','gmw-premium-settings' ); ?>
													</option>
													<option 
														value="smartbox_multiple"  
														<?php if ( $tax_option['style'] == 'smartbox_multiple' ) echo  'selected="selected"'; ?>
													> 
														<?php _e( 'Smartbox ( Multiple selections )','gmw-premium-settings' ); ?>
													</option>
													<option 
														value="checkbox"
														<?php if ( $tax_option['style'] == 'checkbox' ) echo  'selected="selected"'; ?>
													> 
														<?php _e( 'Checkboxes','gmw-premium-settings' ); ?>
													</option>						
												</select>
												<p class="description">
													<?php _e( 'Select the taxonomy usage', 'gmw-premium-settings' ); ?>
												</p>
											</div>
										</div>
											
										<div class="single-option">
											<label><?php _e( 'Include Terms', 'gmw-premium-settings' ); ?></label>
											<div class="tax-content option-content">
												...
												<select 
													...
													name="<?php echo $tax_name_attr; ?>[include][]">
								                	<?php echo GMW_Form_Settings_Helper::get_taxonomy_terms( $taxonomy_name, $include_value ); ?>
								                </select>
								                <p class="description">
													<?php _e( 'Select specific taxonmoy terms to include.', 'gmw-premium-settings' ); ?>
												</p>
											</div>
										</div>

										<div class="single-option">
											<label><?php _e( 'Exclude Terms', 'gmw-premium-settings' ); ?></label>
											<div class="tax-content option-content">
												...
												<select 
													...
													placeholder="Select terms to exclude..." 
													class="taxonomies-picker" 
													name="<?php echo $tax_name_attr; ?>[exclude][]"
												>
								                	<?php echo GMW_Form_Settings_Helper::get_taxonomy_terms( $taxonomy_name, $exclude_value ); ?>
								                </select>
								                <p class="description">
													<?php _e( 'Select specific taxonmoy terms to exclude.', 'gmw-premium-settings' ); ?>
												</p>
											</div>
										</div>
										...
										<div class="taxonomy-enabled-settings">
											...
											<div class="single-option label half">
												<label><?php _e( 'Field label', 'gmw-premium-settings' ); ?></label>	
												<div class="tax-content option-content">
													<input 
														type="text" 
														placeholder="<?php _e( 'Taxonomy label', 'gmw-premium-settings' ); ?>"
														name="<?php echo $tax_name_attr; ?>[label]" 
														value="<?php echo isset( $tax_option['label'] ) ? esc_attr( stripcslashes( $tax_option['label'] ) ) : $tax_label; ?>" 
													/>
													<p class="description">
														<?php _e( 'Enter lable or leave blank to omit.', 'gmw-premium-settings' ); ?>
													</p>
												</div>
											</div>
											...
											<div class="single-option options-all half" style="display:none">
												...
												<div class="tax-content option-content">
													<input 
														type="text" 
														placeholder="<?php _e( 'Options all label', 'gmw-premium-settings' ); ?>" 
														name="<?php echo $tax_name_attr; ?>[show_options_all]" 
														value="<?php echo isset( $tax_option['show_options_all'] ) ? esc_attr( stripcslashes( $tax_option['show_options_all'] ) ) : 'All '. $tax_label; ?>" 
													/>
													<p class="description">
														<?php _e( 'Enter the options all label.', 'gmw-premium-settings' ); ?>
													</p>
												</div>
											</div>

											<div class="single-option orderby half">
												<label><?php _e( 'Order terms by', 'gmw-premium-settings' ); ?></label>
												<?php $selected = ! empty( $tax_option['orderby'] ) ? $tax_option['orderby'] : ''; ?>
												
												<div class="tax-content option-content">
													<select name="<?php echo $tax_name_attr; ?>[orderby]">
														<option value="id" selected="selected"><?php _e( 'ID', 'gmw-premium-settings' ); ?></option>
														<option value="name" <?php if ( $selected == 'name' ) echo 'selected="selected"'; ?>><?php _e( 'Name', 'gmw-premium-settings' ); ?></option>
														<option value="slug" <?php if ( $selected == 'slug' ) echo 'selected="selected"'; ?>><?php _e( 'Slug', 'gmw-premium-settings' ); ?></option>
													</select>
												</div>
											</div>

											<div class="single-option order half">
												<label><?php _e( 'Order', 'gmw-premium-settings'); ?></label>
												<?php $selected = ! empty( $tax_option['order'] ) ? $tax_option['order'] : ''; ?>
												<div class="tax-content option-content">
													<select name="<?php echo $tax_name_attr; ?>[order]">
														<option value="ASC" selected="selected"><?php _e( 'ASC', 'gmw-premium-settings' ); ?></option>
														<option value="DESC" <?php if ( $selected == 'DESC' ) echo 'selected="selected"'; ?>><?php _e( 'DESC', 'gmw-premium-settings' ); ?></option>
													</select>
												</div>
											</div>
											...
											<div class="single-option show-count half">
												<label>
													<input 
														type="checkbox" 
														name="<?php echo $tax_name_attr; ?>[show_count]" 
														value="1" 
														<?php echo ! empty( $tax_option['show_count'] ) ? 'checked="checked"' : ''; ?> 
													/>
													<?php _e( 'Show posts count', 'gmw-premium-settings'); ?>
												</label>				
											</div>
											...
											<div class="single-option hide-empty half">
												<label>	
													<input 
														type="checkbox" 
														name="<?php echo $tax_name_attr; ?>[hide_empty]" 
														value="1" <?php echo ! empty( $tax_option['hide_empty'] ) ? 'checked="checked"' : ''; ?>
													/>
													<?php _e( 'Hide terms without posts', 'gmw-premium-settings'); ?>
												</label>					
											</div>
												...
												<div class="single-option category-icons" style="display:none">
													<label>						
														<input 
															type="checkbox" 
															class="category-icon" 
															name="<?php echo $tax_name_attr; ?>[cat_icons]" 
															value="1" 
															<?php echo ! empty( $tax_option['cat_icons'] ) ? 'checked="checked"' : ''; ?> />
														<?php _e( 'Category icons', 'gmw-premium-settings' ); ?>
													</label>
												</div>	
											<?php } ?>	
										</div>
            ...
            echo '<p>'.__( 'Select a post type to view its taxonomies.', 'gmw-premium-settings' ) .'</p>';
            echo '</div>';
            ...
            echo self::include_exclude_terms( $val, $name_attr . '[include_exclude_terms]', $form );
            //echo '<p>'.__( 'This feature is not availabe with multiple post types.', 'gmw-premium-settings' ) .'</p>';
            echo '</div>';
    ...
    public static function custom_field_generator( $name_attr, $field_name, $field_values, $is_original ) {
    	...
    	?>
    	<div class="single-custom-field-wrapper <?php if ( $is_original ) echo 'original-field'; ?>" data-key="<?php if ( ! $is_original ) echo esc_attr( $field_name ); ?>">						
			...
			<div class="name">
				<input 
					type="text" 
					name="<?php echo $name_attr.'['.$field_name.'][name]'; ?>" 
			...
			<div class="type"> 											
				<select 
					...
					name="<?php echo $name_attr.'['.$field_name.'][type]'; ?>"
				>
					<?php 
				...
				<select 
					...
					name="<?php echo $name_attr.'['.$field_name.'][date_type]'; ?>" 
					<?php if ( $field_values['type'] != 'DATE' ) echo 'style="display:none;"'; ?>
				>
					<option value="yyyy/mm/dd" selected="selected"><?php echo esc_attr( 'Date Type', 'gmw-premium-settings' ); ?></option>
			...
			<div class="compration">
				<select 
					...
					name="<?php echo $name_attr.'['.$field_name.'][compare]'; ?>"
				>
			...
			<div class="label">
				<span>
					<input 
						type="text" 
						name="<?php echo $name_attr.'['.$field_name.'][label][0]'; ?>" 
				...
				<span <?php if ( $field_values['compare'] != 'BETWEEN' && $field_values['compare'] != ' NOT BETWEEN') echo 'style="display:none;"'; ?>>
					<input 
						type="text" 
						name="<?php echo $name_attr.'['.$field_name.'][label][1]'; ?>" 
						value="<?php echo isset( $field_values['label'][1] ) ? esc_attr( stripcslashes( $field_values['label'][1] ) ) : ''; ?>"
						<?php echo $disabled; ?>
						placeholder="<?php _e( 'Second label', 'gmw-premium-settings' ); ?>"
					/>
				</span>
			</div>
			...
			<div class="placeholder">
				<span>
					<input 
						type="text" 
						name="<?php echo $name_attr.'['.$field_name.'][placeholder][0]'; ?>" 
						value="<?php echo isset( $field_values['placeholder'][0] ) ? esc_attr( stripcslashes( $field_values['placeholder'][0] ) ) : ''; ?>"  
						<?php echo $disabled; ?>
						placeholder="<?php _e( 'Placeholder', 'gmw-premium-settings' ); ?>"
					/>
				</span>
				<span <?php if ( $field_values['compare'] != 'BETWEEN' && $field_values['compare'] != ' NOT BETWEEN') echo 'style="display:none;"'; ?>>
					<input 
						type="text" 
						name="<?php echo $name_attr.'['.$field_name.'][placeholder][1]'; ?>" 
						value="<?php echo isset( $field_values['placeholder'][1] ) ? esc_attr( stripcslashes( $field_values['placeholder'][1] ) ) : ''; ?>"
						<?php echo $disabled; ?>
						placeholder="<?php _e( 'Second placeholder', 'gmw-premium-settings' ); ?>"
					/>
				</span>
			</div>
		...
		<div class="gmw-custom-fields-wrapper">
			...
			<div id="header">
				...
				<span class="edit-field-trigger">
					<i class="gmw-icon-cog" onClick="jQuery( '.gmw-custom-fields-wrapper' ).find( '#content, #new-field-picker' ).slideToggle();"></i>
					<?php _e( 'Manage Fields', 'gmw-premium-settings') ;?>		
				</span>

				<div id="new-field-picker" style="display:none;">
					...
					<input 
						...
						value="<?php _e( 'Add field', 'gmw-premium-settings' );?>" 
					/>
				</div>
			</div>
			...
			<div id="content">
				<div class="single-custom-field-wrapper top">
					<div class="name">
			   			<?php echo esc_attr( 'Field','gmw-premium-settings' ); ?>
					</div>
					
					<div class="type"> 											
						<?php echo esc_attr( 'Type','gmw-premium-settings' ); ?>
					</div>

					<div class="compration">
						<?php echo esc_attr( 'Compration','gmw-premium-settings' ); ?>
					</div>

					<div class="label">
						<?php echo esc_attr( 'Label','gmw-premium-settings' ); ?>
					</div>

					<div class="placeholder">
						<?php echo esc_attr( 'Placeholder','gmw-premium-settings' ); ?>
					</div>
    			</div>

				<div id="custom-fields-holder">

					<?php self::custom_field_generator( 
						$name_attr, 
						'%%field_name%%', 
						array( 
							'name' 	  => '', 
					...
					<?php if ( ! empty( $value ) ) { ?>
						<?php foreach ( $value as $field_name => $field_values ) {
	...
	public static function include_exclude_terms( $value, $name_attr, $form_fields ) {
		...
		if ( empty( $form_fields['search_form']['post_types'] ) ) {
			echo '<p>' . __( 'You need to select at least one post type in the search form tab to be able to use this feature.' ) . '</p>';
			return;
		} 
			...
			<div class="gmw-taxonomies-picker-wrapper <?php echo esc_attr( $post_type->name ); ?>" style="display:none">
		        ...
		        <?php if ( empty( $taxonomies ) ) { ?>
		        	...
		        	<div class="header">
			        	...
			        	<span><?php _e( '- no taxonomies found.', 'gmw-premium-settings' ); ?></span>
			        </div>
			        ...
			        <div class="single-taxonomy-picker gmw-options-box <?php echo esc_attr( $post_type->name ); ?>">
			        	<ul class="options-tabs">
					        <?php foreach ( $taxonomies as $taxonomy ) { ?>
					        	<li class="option-tab <?php echo esc_attr( $taxonomy->name ); ?> <?php if ( $count == 0 ) echo 'active'; ?>">
						            ...
						            <div class="single-option">
						                ...
						                <div class="option-content">
							                ...
							                <select 
							                	multiple 
							                	class="taxonomies-picker" 
							                	name="<?php echo esc_attr( $name_attr.'['.$taxonomy->name.'][include][]' ); ?>"
							                >
							                	<?php echo GMW_Form_Settings_Helper::get_taxonomy_terms( $taxonomy->name, $selected ); ?>
							                </select>
							        	</div>
							        </div>
	 ...
	 * @return [type]         [description]
	 */
	public static function validate_include_exclude_terms( $output ) {
