...
class GMW_PS_PT_Admin_Settings {
	...
	public function settings( $settings ) {
		...
		$settings['location_form_options'] = array(
			...
			'type'    		=> 'fields_group',
			'label'   		=> __( 'Location Sections', 'gmw-premium-settings' ),
			'desc'     		=> __( 'Setup the Location section of the admin\'s "Edit post" page.', 'gmw-premium-settings' ),
			'fields'		=> array(
	...
	public function post_types_icons( $value, $name_attr ) {
		...
		if ( empty( $post_types ) ) {
			...
			echo '<em>'._e( 'You need to select at least 1 post type above in order to use this feature.', 'gmw-premium-settings' ) .'</em>';
		...
		<div id="post-types-map-icons-wrapper" style="width:100%;max-width: 500px;">
			...
			foreach ( $post_types as $post_type ) { ?>
				...
				<?php 
					if ( ( $pt_object = get_post_type_object( $post_type ) ) == false ) {
						continue;
					}
				?>
				...
				<div class="post-type-map-icons-wrapper">
					...
					<div class="content">
						...
						foreach ( $map_icons as $map_icon ) {
							$checked = ( ( isset( $value[$post_type] ) && $value[$post_type] == $map_icon ) || $cic == 1 ) ? 'checked="checked"' : '';
							echo '<label><input type="radio" name="'.$name_attr.'['.esc_attr( $post_type ).']" value="'.esc_attr( $map_icon ).'" '.$checked.' />';
							echo '<img src="'.esc_url( $icons_url.$map_icon ).'" />';
							//echo '<span> ' . esc_attr( pathinfo( $map_icon, PATHINFO_EXTENSION ) ) . '</span></label>';
							echo '</label>';
							$cic++;
						}				
						?>
		...
		<div class="gmw-description-box" style="width:100%;max-width: 500px;margin:0;">
			...
			<p><?php _e( '* Make sure to click "Save Changes" after modfing the Post Types settings above to refresh the list of taxonomies icons below.', 'gmw-premium-settings' ); ?></p>
	...
	public function per_category_icons( $value, $name_attr, $gmw_options ) {
		...
		<label>
			<input 
				...
				<?php echo $checked; ?>
			/>
			<?php _e( 'Enable category icons', 'gmw-premium-settings' ); ?>
		</label>
		</p>

		<?php $checked = isset( $value['same_icons'] ) ? 'checked="checked"' : ''; ?>

		<div class="gmw-options-box per-category-icons-trigger" <?php echo $display; ?>>
			
			<?php 
			/* temporary disabled. This feature seems unecessary. 
			Most cases users should use same icons form map and checkboxes.
			This might be re-enabled per users request.

			<div class="single-option">
				<label>
					<input 
						class="setting-per_category_icons_same_icons" 
						name="<?php echo $name_attr.'[same_icons]' ;?>" 
						type="checkbox" value="1" 
						<?php echo $checked; ?>
					/>
					<?php _e( 'Use same icons for category and map icons', 'gmw-premium-settings' ); ?>
				</label>
			</div>
			*/ ?>
			...
			<div class="single-option checkbox">
				...
				<label><b><?php _e( 'Taxonomies', 'gmw-premium-settings' ); ?></b></label>
				...
				<div class="option-content">
					...
					foreach ( get_object_taxonomies( $gmw_options['post_types_settings']['post_types'] ) as $taxonomy ) { 
						...
						<label>
							<input 
								...
								name="<?php echo $name_attr.'[taxonomies][]' ;?>" 
					...
					?>
					<p class="description">
						<?php _e( 'Check the taxnomies to enable map icons.', 'gmw-premium-settings' ); ?>
					
						<span style="font-size:12px;display:block;line-height: 18px;color:red;">
							<?php _e( '* Make sure to click "Save Changes" after modfing the Post Types settings above to refresh the list of taxonomies icons below.', 'gmw-premium-settings' ); ?>
						</span>
					</p>
				</div>
			</div>
			...
			<div class="single-option">
				...
				<label><b><?php _e( 'Taxonomies term orderby', 'gmw-premium-settings' ); ?></b></label>
				
				<div class="option-content">
					<select name="<?php echo $name_attr.'[terms_orderby]' ;?>">
						
						<?php $orderby_value = ( !empty( $value['terms_orderby'] ) ) ? $value['terms_orderby'] : ''; ?>

						<option value="term_id" selected="selected"><?php _e( 'Term ID', 'gmw-premium-settings'); ?></option>
						<option value="parent" <?php selected( $orderby_value, 'parent' ); ?>><?php _e( 'Parent', 'gmw-premium-settings'); ?></option>
						<option value="name" <?php selected( $orderby_value, 'name' ); ?>><?php _e( 'Name', 'gmw-premium-settings'); ?></option>
						<option value="count" <?php selected( $orderby_value, 'count' ); ?>><?php _e( 'Count', 'gmw-premium-settings'); ?></option>
						<option value="slug" <?php selected( $orderby_value, 'slug' ); ?>><?php _e( 'Slug', 'gmw-premium-settings'); ?></option>
						<option value="term_group" <?php selected( $orderby_value, 'term_group' ); ?>><?php _e( 'Term group', 'gmw-premium-settings'); ?></option>
						<option value="taxonomy" <?php selected( $orderby_value, 'taxonomy' ); ?>><?php _e( 'Taxonomy', 'gmw-premium-settings'); ?></option>
						<option value="term_taxonomy_id" <?php selected( $orderby_value, 'term_taxonomy_id' ); ?>><?php _e( 'Term taxonomy id', 'gmw-premium-settings'); ?></option>
						<option value="term_order" <?php selected( $orderby_value, 'term_order' ); ?>><?php _e( 'Term order', 'gmw-premium-settings'); ?></option>
					</select>
					<p class="description">
						<?php _e( 'When a post have multiple cateogies, the orderby will determine which category icon will be used on the map based on the terms order.', 'gmw-premium-settings' ); ?>	
					</p>
				</div>
			</div>

			<div class="single-option">
				<label><b><?php _e( 'Taxonomies term order', 'gmw-premium-settings' ); ?></b></label>
				<div class="option-content">
					<select name="<?php echo $name_attr.'[terms_order]' ;?>">
						<?php $order_value = ! empty( $value['terms_order'] ) ? $value['terms_order'] : ''; ?>
						<option value="DESC" selected="selected"><?php _e( 'DESC', 'gmw-premium-settings'); ?></option>
						<option value="ASC"  <?php selected( $order_value, 'ASC' );  ?>><?php _e( 'ASC', 'gmw-premium-settings'); ?></option>
					</select>
				</div>
			</div>
			...
			<div class="single-option">
				...
				<label><b><?php _e( 'Default icon', 'gmw-premium-settings' ); ?></b></label>
				...
				<div class="option-content form-field term-category-icons-wrap">
					...
					<div class="category-icons icons" style="display: inline-block">
						...
						foreach ( $category_icons as $category_icon ) {
							...
							if ( $cic == 1 || ( ! empty( $value['default_icon'] ) && $value['default_icon'] == $category_icon ) ) {
								$checked =  'checked="checked"';
							}

							echo '<label style="width:initial !important">';
							echo '<input type="radio" name="'.$name_attr.'[default_icon]'.'" value="'.esc_attr( $category_icon ).'" '.$checked.'/>';
					...
					<p class="description"><?php _e( 'Select the icon that will be used when no icon found for specific category or post.', 'gmw-premium-settings' ); ?>
					</p>
			</div>
