...
function gmw_disable_keywords_content_search( $gmw ) {
	...
	if ( isset( $gmw['search_form']['keywords']['usage'] ) && 'title' == $gmw['search_form']['keywords']['usage'] ) {
...
function gmw_ps_pt_query_keywords( $where, $wp_query ) {
	...
	if ( ( $gmw_args = $wp_query->get( 'gmw_args' ) ) == false ) {
		return $where;
	}
...
function gmw_ps_pt_orderby_query( $args, $form ) {
	...
	if ( ( $value = gmw_ps_get_orderby_value( $form ) ) == '' ) {
		return $args;
	}
	...
	if ( $value == 'distance' ) {
		...
		if ( $args['orderby'] != 'post_modified' && $args['orderby'] != 'post_date' ) {
			$args['order']   = 'ASC';
		}
	}
...
/**
	 * Include / exclude taxonomy terms on page load
 *
 * @since 1.0
 *
 ...
 *
 */
/*function gmw_ps_query_pre_defined_taxonomies( $clauses, $gmw ) {
	...
	// on form submission
		...
		// abort if multiple post types set in the form.
        if ( ! isset( $gmw['search_form']['post_types'] ) || count( $gmw['search_form']['post_types'] ) != 1 ) {
            return $clauses;   
        }
	...
	foreach ( $tax_args as $taxonomy => $args ) {
		...
		if ( ! empty( $args['include'] ) ) {
			$include_set = true;
			$include 	 = array_merge( $include, $args['include'] );
		}
	...
	if ( $include_set ) {
		...
		// escape terms ID
		$terms_id = esc_sql( $include );
		$terms_id = implode( ',', $terms_id );

		//$include_sql = " AND tt.term_id IN ( '{$terms_id}' ) ";

		$clauses['where'] .= " AND ( {$wpdb->prefix}posts.ID IN (
			SELECT object_id 
			FROM {$wpdb->prefix}term_relationships tr
			INNER JOIN {$wpdb->prefix}term_taxonomy tt
			ON tr.term_taxonomy_id = tt.term_taxonomy_id
			WHERE tt.term_id IN ( '{$terms_id}' )
	...
	/*if ( ! empty( $include ) ) {
		...
		$clauses['where'] .= " AND ( {$wpdb->prefix}term_relationships.term_taxonomy_id IN ( {$terms_id} ) )";
		$clauses['join']  .= " INNER JOIN {$wpdb->prefix}term_relationships ON ( {$wpdb->prefix}posts.ID = {$wpdb->prefix}term_relationships.object_id ) ";
	}
	...
	if ( ! empty( $exclude ) ) {
		...
		$clauses['where'] .= " AND ( {$wpdb->prefix}posts.ID NOT IN (
			...
			WHERE term_taxonomy_id IN ( {$terms_id} ) ) ) ";
	} */

	/*return $clauses;
}*/
//add_filter( 'gmw_pt_location_query_clauses', 'gmw_ps_query_pre_defined_taxonomies', 15, 2 );

/**
 * Query include/exclude taxonomies 
 * 
 * @param  array $output tax_query array to pass to WP_Query
...
//add_filter( 'gmw_pt_query_taxonomy', 'gmw_ps_pt_query_taxonomies', 10, 4 );
 ...
 *
 * @param  [type] $meta_args [description]
 * @param  [type] $gmw       [description]
 * @return [type]            [description]
 *
 * @since 1.5
 */
function gmw_ps_pt_get_meta_query_args( $custom_fields, $values ) {
...
function gmw_ps_pt_query_custom_fields( $gmw ) {
	...
	$meta_args = gmw_ps_pt_get_meta_query_args( $gmw['search_form']['custom_fields'], $gmw['form_values']['cf'] );

	$gmw['query_args']['meta_query'] = $meta_args;
