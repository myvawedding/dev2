...
function gmw_search_form_checkboxes_options_selector( $output, $args, $options ) {
	...
	foreach ( $options as $value => $name ) {
		...
		$checked = ( isset( $_GET[ $name_tag ] ) && in_array( $value, $_GET[ $name_tag ] ) ) ? 'checked="checked"' : '';
...
function gmw_get_search_form_keywords_field( $gmw ) {
	...
	/**
	 * Dynamically append keywords field to the search form
	 * voa action hook
	 *
...
function gmw_search_form_keywords_field( $gmw = array() ) {
	...
	add_action( 'gmw_search_form_start', 'gmw_search_form_keywords_field' );
...
function gmw_get_search_form_address_fields( $output, $gmw ) {
	...
	foreach ( $gmw['search_form']['address_field']['multiple'] as $field_name => $field_args ) {
		...
		if ( 'default' === $usage ) {
			...
			$value   = ! empty( $_GET[ $url_px . 'address' ][ $field_name ] ) ? esc_attr( stripslashes( $_GET[ $url_px . 'address' ][ $field_name ] ) ) : '';
...
function gmw_search_form_address_fields( $gmw ) {
	echo gmw_get_search_form_address_fields( $gmw );
}
...
function gmw_get_search_form_radius_slider( $gmw ) {
	...
	if ( ! empty( $gmw['search_form']['radius_slider']['label'] ) ) {
		$output .= '<label class="gmw-field-label>' . esc_attr( $gmw['search_form']['radius_slider']['label'] ) . '</label>';
	}
	...
	/**
	 * Output radius slide in a search form
	 *
	 * @param  array  $args [description]
	 * @return [type]       [description]
	 */
function gmw_search_form_radius_slider( $args = array() ) {
