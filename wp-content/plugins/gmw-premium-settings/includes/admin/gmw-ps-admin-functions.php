...
function gmw_refresh_map_icons_button( $page = '', $message = true ) {
	...
	<div class="gmw-refresh-icons-wrapper">
		<?php if ( $message ) { ?>
		<em>
			<?php _e( '* Use the "Refresh Icons" button after uploading new icons or if map icons are missing.', 'gmw-premium-settings' ); ?>	
		</em>
		<?php } ?>
    	<input type="button" class="button-secondary" onclick="location.href='<?php echo $url; ?>';" value="<?php _e( 'Refresh Icons', 'gmw-premium-settings' ); ?>" />
	</div>
	<?php
}
...
function gmw_refresh_map_icons( $page ) {
	...
	if ( ! empty( $_GET['action'] ) && $_GET['action'] == 'refresh_map_icons' && check_admin_referer( 'gmw_map_icons_refresh_nonce' ) ) {
		...
		if ( ! empty( $_GET['page'] ) ) {
			...
			$action  = ! empty( $_GET['gmw_action'] ) ? '&gmw_action=' . $_GET['gmw_action'] : '';
			$form_id = ! empty( $_GET['form_id'] ) ? '&form_id=' . $_GET['form_id'] : '';
			$page 	 = 'admin.php?page='.$_GET['page'].$action.$form_id;

		} elseif ( ! empty( $_GET['taxonomy'] ) ) {
			$page = 'edit-tags.php?taxonomy='.$_GET['taxonomy'];
		} else {
...
function gmw_ps_collect_icons() {
	...
	if ( gmw_is_addon_active( 'posts_locator' ) ) {
		...
		if ( is_dir( STYLESHEETPATH."/geo-my-wp/{$posts_locator_data['templates_folder']}/map-icons/" ) ) {
			$pt_map_icons_path = STYLESHEETPATH."/geo-my-wp/{$posts_locator_data['templates_folder']}/map-icons/";
		...
		if ( is_dir( STYLESHEETPATH."/geo-my-wp/{$posts_locator_data['templates_folder']}/category-icons/" ) ) {
			$pt_category_icons_path = STYLESHEETPATH."/geo-my-wp/{$posts_locator_data['templates_folder']}/category-icons/";
	...
	foreach ( $addons as $addon ) {
		...
		$map_icons_path = $map_icons_url = $map_icons = false;

		// get addon's data
		$addon_data = gmw_get_addon_data( $addon );

		if ( is_dir( STYLESHEETPATH."/geo-my-wp/{$addon_data['templates_folder']}/map-icons/" ) ) {
			$map_icons_path = STYLESHEETPATH . "/geo-my-wp/{$addon_data['templates_folder']}/map-icons/";
