 ...
 *
 *
 * @function in geo-my-wp/includes/template-functions/gmw-search-results-template-functions.php
 *
...
/**
 * Prevent form from collecting info window content
 *
 * when using ajax for info-windows.
 *
...
function gmw_ps_info_window_args( $args, $location, $gmw ) {
	...
	$args['directions_link'] = ( isset( $settings['directions_link'] ) && $settings['distance'] ) ? true : false;
	$args['location_meta']   = ! empty( $settings['location_meta'] ) ? $settings['location_meta'] : false;

	if ( empty( $settings['image']['enabled'] ) ) {
		$args['image_url'] = $args['image'] = false;
	}
 ...
 *
 * @param  string $no_results original message
...
function gmw_ps_no_results_message( $message, $gmw ) {
	...
	if ( strpos( $message, '{wider_search_link}' ) !== false && ! empty( $form_field['wider_search']['link_text'] ) ) {
		...
		$link = '<a href="' . esc_url( $url ) . '" onclick="document.gmw_form.submit();">' . esc_attr( stripslashes( $form_field['wider_search']['link_text'] ) ) . '</a> ';
	...
	if ( strpos( $message, '{all_results_link}' ) !== false && ! empty( $form_field['all_results_link'] ) ) {
		...
		if ( 1 == 1 ) {
			...
			$link = '<a href="' . esc_url( $url ) . '" onclick="document.gmw_form.submit();">' . esc_attr( stripslashes( $form_field['all_results_link'] ) ) . ' </a>';
		} else {

			$link = '<a href="#"  onclick="alert(1)">' . esc_attr( stripslashes( $form_field['all_results_link'] ) ) . ' </a>';
		}
...
function gmw_ps_modify_map_elements( $map_elements, $gmw ) {
	...
	$map_elements['map_options'] = array_merge(
		$map_elements['map_options'], array(
			'zoomControl'        => false,
			'rotateControl'      => false,
...
function gmw_ps_modify_results_address_fields( $address, $location, $gmw ) {
	...
	} elseif ( in_array( 'disabled', $gmw['search_results']['address_fields'] ) ) {

		return false;

		// full address
	} elseif ( in_array( 'address', $gmw['search_results']['address_fields'] ) ) {
...
/**
 * Get the orderby value on page load and formsubmission
 *
