...
class GMW_Premium_Settings_Addon extends GMW_Addon {
	...
	public function global_maps_features() {
		...
		// include gmaps features
		//include( 'includes/gmw-ps-gmaps-functions.php' );
	}
	...
	public function enqueue_scripts() {
		...
		wp_register_style( 'datetime-picker', GMW_URL . '/assets/lib/pickadate/datetime.picker.classic.min.css', array(), GMW_VERSION );
		wp_register_script( 'datetime-picker', GMW_URL . '/assets/lib/pickadate/datetime.picker.min.js', array( 'jquery' ), GMW_VERSION, true );
		...
		if ( IS_ADMIN ) {
			...
			if ( 'chosen' === $smartbox_library ) {
				...
				if ( ! wp_style_is( 'select2', 'registered' ) ) {
					wp_register_style( 'select2', GMW_URL . '/assets/lib/select2/css/select2.min.css', array(), GMW_VERSION );
				}

				if ( ! wp_script_is( 'select2', 'registered' ) ) {
					wp_register_script( 'select2', GMW_URL . '/assets/lib/select2/js/select2.min.js', array( 'jquery' ), GMW_VERSION, true );
				}
			}
		}
	}
