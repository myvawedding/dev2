...
add_action('admin_notices', array('Frontend_Checklist_Menu', 'adminNotice')); 
...
class Frontend_Checklist_Menu {
	...
	static public function overviewPage() {
		...
		<p><?php _e('If you have questions abot the plugin or something is not working, <a href="http://www.j-breuer.com/contact/" target="_blank">send me a message</a>. I\'m always happy to help.', 'frontend-checklist'); ?></p>
	...
	static public function adminNotice() {
	    $current_screen = get_current_screen();
	    if ($current_screen->base != 'settings_page_frontend-checklist') return;
	    
	    $meta_options = get_option('frontend-checklist-meta-options');
	    if (!isset($meta_options['infotext']) || empty($meta_options['infotext']) || !isset($meta_options['hide-infotext']) || $meta_options['hide-infotext'] != 0) return;
	    
	    echo '<div class="notice notice-warning"><p>'.$meta_options['infotext'].'</p></div>';
	}
